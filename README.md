...
## Changelog
1. Thread layout — tree view by default
...
2. Reply links rendered under posts
...
3. OP gets a full-width card with a thin gradient accent bar on the left edge and a soft glow. Replies use a glassmorphism bubble (backdrop-filter: blur) with a very       subtle border — lighter visual weight, distinct from the OP. The hover-reveal action buttons (hide) appear on hover only, keeping things clean.
